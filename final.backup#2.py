...
for vehicle in uvmVehicles.response.json().get('data').get('603'):
    if uvmVehicles.response.json().get('data').get('603')[count5].get('route_id') == '4006422':
        vehicles.append('On-Campus ' + str(count4))
        count5 += 1
        count4 += 1
    elif uvmVehicles.response.json().get('data').get('603')[count5].get('route_id') == '4006426':
        vehicles.append('Redstone express' + str(count6))
        count5 += 1
        count6 += 1 
...
while True:
    if GPIO.input(BUTTON1) == GPIO.LOW:
        ...
        show_message(device, msg, fill="white", font=proportional(CP437_FONT))
        count = count + 1
    if GPIO.input(BUTTON2) == GPIO.LOW and 'Redstone' in vehicles[(count-1) % len(vehicles)]:
    ...
    if GPIO.input(BUTTON3) == GPIO.LOW and 'Redstone' in vehicles[(count-1) % len(vehicles)]:
    ...
    if GPIO.input(BUTTON4) == GPIO.LOW:
        ...
        response = requests.get(url, headers=headers, params=querystring)
        msg = str(response.json().get('data').get('603')[(count-1)%len(vehicles)].get('speed')) + 'mph'
        show_message(device, msg, fill="white", font=proportional(CP437_FONT))
